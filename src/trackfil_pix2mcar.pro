PRO trackfil_pix2mcar, hfc_data, ske_pos, ds=ds,$
...
;
; CALLING SEQUENCE:
;		IDL> trackfil_pix2mcar,hfc_data,ske_pos
;
; INPUTS:
;		hfc_data - IDL structure containing the filaments data loaded in the HFC database.
;
; OPTIONAL INPUTS:
...
if (size(hfc_data,/TNAME) ne 'STRUCT') then message,'DATA input parameter must be a structure!'
